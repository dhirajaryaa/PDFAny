...
- **PWA**: `public/manifest.webmanifest` (standalone, icons `icon-192.png`/`icon-512.png`/`icon-maskable-512.png` derived from the logo via magick), `public/sw.js` (precache `/`, offline.html, manifest, logo; network-first nav with `.html` fallback for clean URLs; cache-first for `/_next/static` + assets; runtime-caches visited pages for offline use), `public/offline.html`. Registered by `src/components/PwaRegister.tsx` (client, `void navigator.serviceWorker.register("/sw.js")`). Root layout sets `metadata.manifest`, `appleWebApp` + `formatDetection` (Next 16 moved these to `Metadata`, NOT `Viewport`).
